...
public abstract class HTTPDataCallback implements  com.linghushaoxia.http.parse.IHTTPDataCallback{
	/*
	...
	*/
	public int cb(com.linghushaoxia.http.parse.HTTPParser p, ByteBuffer buf, int pos, int len) {
	  byte [] by = new byte[len];
    int saved = buf.position();
    buf.position(pos);
    buf.get(by);
    buf.position(saved);
    return cb((HTTPParserDefault)p, by, 0, len);
	}

  public abstract int cb(HTTPParserDefault p, byte[] by, int pos, int len);
}
